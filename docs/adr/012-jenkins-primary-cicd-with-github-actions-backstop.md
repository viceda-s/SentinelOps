...
* A smee.io tunnel and a running Jenkins stack are both required for real-time PR status checks; if either is down, only the GitHub Actions backstop check reports. The tunnel now runs as a `smee` service in `docker-compose.yml` (`docker/smee/Dockerfile`, `restart: unless-stopped`) rather than a manually-started process, after the manual version repeatedly went down whenever the host machine restarted or the terminal it ran in was closed. `smee-client@4.4.3` is pinned (not `5.0.0`, which reintroduces the same `sec-fetch-mode`/CORS-preflight 405 bug against Jenkins's webhook endpoint that `@2` originally avoided — confirmed still open as of `probot/smee-client` issue #439, filed against `5.0.0` in March 2026; `4.4.3` is the newest version confirmed not to exhibit it).
