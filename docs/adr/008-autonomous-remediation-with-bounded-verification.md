...
* `VERIFY_TIMEOUT = 30` seconds — default time allowed for a verification attempt. For `docker-health` verification, `_verify_timeout_for()` extends this per attempt to cover the target container's own `HEALTHCHECK` start period, interval, and timeout (plus a fixed margin), up to a hard ceiling of `HEALTHCHECK_VERIFY_MAX = 60` seconds — see Consequences. The worker's heartbeat is refreshed on every verification poll tick, so a long verify budget does not by itself starve the `ResponseEngineDown` liveness check.
