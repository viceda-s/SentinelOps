...
- The rest of the remediation worker: the `collect_diagnostics` playbook, and `worker.py`'s actual polling loop/dispatch that ties `claim_incident()` to a playbook — right now `restart_service` only runs when called directly, there's no process wiring the queue to it yet. The report generator.
